Create one horizontal animation strip for Codex pet `selection-layout`, state `running-right`.
...
Identity: same pet in every frame: Cute chibi black-and-white pixel pet with compact whole-body silhouette.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `pixel`: Pixel-art-adjacent digital mascot with a chunky silhouette, simple dark outline, limited palette, flat cel shading, and visible stepped edges. User style notes: Pure black and pure white only; no gray; nearest-neighbor pixel art; large readable face..
